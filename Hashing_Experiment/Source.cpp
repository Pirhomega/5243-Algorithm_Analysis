...
int main() {
    ...
    // probe_count_linear and probe_count_double will accumulate the table's probes over all
	// insertions test1 and test2 are two hash tables. Test1's CRP is linear probing and 
	// test2's is double hashing table_array will store all the hash tables for ease of use
    // NOTE: table_array will become more useful in the full program. That's why it's use is
	// 		pointless in this initial version turned in on March 10th.
    ...
    // read from input file and insert all values read to the two test hash tables all while
    // accumulating probes for each insertion
    while(infile >> key) {
...
// Print_Tables will call each hash table's print function as well as print headers and 
// average probes
// NOTE: "Avg. Probes #2" is not utilized in this early version so its value will always be
// zero. Therefore, "Avg. of 2 runs" will be the same value as "Avg. Probes #1"
void Print_Tables(std::ofstream& outf, std::vector<Hash>& tables, double lin_avg, 
	double doub_avg) {
    outf << std::setprecision(3) << std::fixed << std::setw(20) << "Description" 
		<< std::setw(20) 
		<< "Avg. Probes #1" << std::setw(20) << "Avg. Probes #2" << std::setw(20) 
		<< "Avg. of 2 runs\n"
        << "--------------------------------------------------------------------------------"
		<< "\n"
        << std::setw(20) << "Linear Probing" << std::setw(20) <<  lin_avg << std::setw(20)
		<< 0 << std::setw(20) << lin_avg << '\n'
        << std::setw(20) << "Double Hashing" << std::setw(20) <<  doub_avg << std::setw(20)
		<< 0 << std::setw(20) << doub_avg << '\n'
        << "--------------------------------------------------------------------------------"
		<< "\n\n"
